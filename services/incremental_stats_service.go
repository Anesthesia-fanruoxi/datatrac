...
// MergeFullSyncProgress 合并全量同步进度到表统计
func (s *IncrementalStatsService) MergeFullSyncProgress(taskID string, statsList []*IncrementalTableStats, progressService *TaskProgressService) []*IncrementalTableStats {
	// 从内存获取任务进度
	...
	// 创建表名到运行时数据的映射
	unitMap := make(map[string]*struct {
		TotalRecords     int64
	...
	for _, unit := range units {
		...
		if unit.TotalRecords > 0 {
			unitMap[unit.UnitName].Progress = float64(unit.ProcessedRecords) / float64(unit.TotalRecords) * 100
		}

		// 调试日志
		fmt.Printf("[DEBUG] UnitName: %s, TotalRecords: %d, ProcessedRecords: %d\n",
			unit.UnitName, unit.TotalRecords, unit.ProcessedRecords)
	}
	...
	for _, stats := range statsList {
		...
		// 调试日志
		fmt.Printf("[DEBUG] 尝试匹配表: %s\n", tableName)

		if unitData, exists := unitMap[tableName]; exists {
			stats.FullSyncTotalRecords = unitData.TotalRecords
			stats.FullSyncProcessedRecords = unitData.ProcessedRecords
			stats.FullSyncProgress = unitData.Progress

			// 调试日志
			fmt.Printf("[DEBUG] 匹配成功! TotalRecords: %d, ProcessedRecords: %d, Progress: %.2f%%\n",
				unitData.TotalRecords, unitData.ProcessedRecords, unitData.Progress)
		} else {
			// 调试日志
			fmt.Printf("[DEBUG] 匹配失败! 可用的 UnitName: %v\n", getMapKeys(unitMap))
		}
	}
...
func getMapKeys(m map[string]*struct {
	TotalRecords     int64
	ProcessedRecords int64
	Progress         float64
}) []string {
