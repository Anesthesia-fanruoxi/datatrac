    ...
    window.TaskConfigCore = {
        ...
        getStatusBadge: function(status) {
            const badges = {
                ...
                'running': '<span class="badge bg-success"><i class="bi bi-play-fill me-1"></i>运行中</span>',
                'paused': '<span class="badge bg-warning">已暂停</span>',
                'completed': '<span class="badge bg-primary"><i class="bi bi-check-circle-fill me-1"></i>已完成</span>',
        ...
        view: async function(taskId) {
            try {
                const result = await HttpUtils.get(`/api/v1/tasks/${taskId}/config-view`);
                ...
                const configView = result.data;
                const task = configView.task;
                const config = configView.config || {};
                
                // 显示查看模态框
                this.showViewModal(task, config, configView.real_time_info);
        ...
        // 显示查看模态框
        showViewModal: function(task, config, realTimeInfo) {
                ...
                <div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); background:white; padding:0; border-radius:15px; width:70%; max-width:1000px; max-height:80vh; display: flex; flex-direction: column; box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
                    ...
                    <div style="padding: 30px; flex: 1; overflow-y: auto;">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <h6>选中的表 <small class="text-success">(显示当前数据库中的实际表名)</small></h6>
                                <div style="max-height: 300px; overflow-y: auto;">
                                    ${this.renderTableListWithRealTime(config.selected_databases, realTimeInfo)}
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        // 渲染表列表（包含实时表结构信息）
        renderTableListWithRealTime: function(databases, realTimeInfo) {
            if (!databases || databases.length === 0) {
                return '<div class="text-muted">暂无表配置</div>';
            }
            
            return databases.map(db => {
                const realTimeTables = realTimeInfo && realTimeInfo[db.source_database] ? realTimeInfo[db.source_database] : [];
                const realTimeTableSet = new Set(realTimeTables);
                
                const dbDisplayName = db.is_database_modified ? 
                    `${db.database} <span class="text-muted" style="font-size:0.85em;">(原: ${db.source_database})</span>` : 
                    db.database;
                
                const tables = db.tables?.map(t => {
                    const exists = realTimeTableSet.has(t.source_table);
                    const statusIcon = exists ? 
                        '<i class="bi bi-check-circle-fill text-success me-1" title="表存在"></i>' : 
                        '<i class="bi bi-exclamation-triangle-fill text-warning me-1" title="表可能已被删除或重命名"></i>';
                    const textClass = exists ? '' : 'text-muted';
                    
                    const tableDisplayName = t.is_modified ? 
                        `${t.target_table} <span class="text-muted" style="font-size:0.85em;">(原: ${t.source_table})</span>` : 
                        t.source_table;
                    
                    return `<li class="${textClass}">${statusIcon}${tableDisplayName}</li>`;
                }).join('') || '';
                
                return `
                    <div class="mb-2">
                        <strong><i class="bi bi-database me-1"></i>${dbDisplayName}</strong>
                        <ul class="mb-0" style="list-style: none; padding-left: 1.2rem;">${tables}</ul>
                    </div>
                `;
            }).join('');
        },
