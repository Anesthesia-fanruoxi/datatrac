...
(function() {
    ...
    window.TaskMonitorLogsSSE = {
            ...
            this.eventSource.addEventListener('log', (e) => {
                try {
                    ...
                    if (Array.isArray(logs)) {
                        logs.forEach(log => {
                            // 优先调用抽屉组件的方法
                            if (window.TaskMonitorDrawer && typeof window.TaskMonitorDrawer.appendLog === 'function') {
                                window.TaskMonitorDrawer.appendLog(log);
                            } else if (window.TaskMonitorDetail && typeof window.TaskMonitorDetail.appendLog === 'function') {
        ...
        switchCategory: function(taskId, category) {
            // 清空日志显示
            if (window.TaskMonitorDrawer && typeof window.TaskMonitorDrawer.clearLogs === 'function') {
                window.TaskMonitorDrawer.clearLogs();
            } else if (window.TaskMonitorDetail && typeof window.TaskMonitorDetail.clearLogs === 'function') {
