...
type TaskSSEService struct {
	...
	progressClients map[string]map[chan SSEMessage]ProgressClientParams // taskID -> client -> params (存储每个客户端的database和targetID参数)
...
// ProgressClientParams 进度客户端参数
type ProgressClientParams struct {
	Database string
	TargetID string
}
...
func NewTaskSSEService() *TaskSSEService {
	taskSSEOnce.Do(func() {
		taskSSEInstance = &TaskSSEService{
			...
			progressClients: make(map[string]map[chan SSEMessage]ProgressClientParams),
...
// AddProgressClient 添加进度客户端（带 database 和 targetID 参数）
func (s *TaskSSEService) AddProgressClient(taskID string, client chan SSEMessage, dbName string, targetID string) {
	s.mu.Lock()
	defer s.mu.Unlock()

	if s.progressClients[taskID] == nil {
		s.progressClients[taskID] = make(map[chan SSEMessage]ProgressClientParams)
	}
	s.progressClients[taskID][client] = ProgressClientParams{
		Database: dbName,
		TargetID: targetID,
	}
}
...
func (s *TaskSSEService) StreamProgress(taskID string, dbName string, targetID string, client chan SSEMessage, done <-chan struct{}) {
	// 立即发送一次初始数据（带参数）
	s.sendProgressUpdate(taskID, dbName, targetID, client)
...
func (s *TaskSSEService) sendTaskDetailUpdate(taskID string, client chan SSEMessage) {
	...
	// 从配置中解析目标源列表
	targetIDs := s.parseTargetIDsFromConfig(&task)
	if len(targetIDs) > 0 {
		targets := make([]map[string]string, 0, len(targetIDs))
		dsService := NewDataSourceService()
		for _, tID := range targetIDs {
			if ds, err := dsService.GetByID(tID); err == nil && ds != nil {
		...
		if len(targets) > 0 {
			taskDetail["target_conns"] = targets
		}
	} else if task.TargetID != "" {
		// 单目标源回退
		dsService := NewDataSourceService()
		if ds, err := dsService.GetByID(task.TargetID); err == nil && ds != nil {
			taskDetail["target_conns"] = []map[string]string{
...
// parseTargetIDsFromConfig 从任务配置中解析目标源ID列表
func (s *TaskSSEService) parseTargetIDsFromConfig(task *models.SyncTask) []string {
	var config struct {
...
// sendProgressUpdate 发送统一的进度更新（带参数）
func (s *TaskSSEService) sendProgressUpdate(taskID string, dbName string, targetID string, client chan SSEMessage) {
	defer func() {
		if r := recover(); r != nil {
			// channel 已关闭，忽略错误
		}
	...
	// 获取进度（传递参数）
	progress, err := s.progressService.GetTaskProgress(taskID, targetID, dbName)
	if err == nil {
		select {
		case client <- SSEMessage{
			Event: "progress",
...
func (s *TaskSSEService) BroadcastProgressUpdate(taskID string) {
	...
	// 向所有进度客户端发送更新
	sentCount := 0
	for client, params := range clients {
		func(c chan SSEMessage, p ProgressClientParams) {
			defer func() {
				if r := recover(); r != nil {
					// channel 已关闭，忽略错误
				}
			...
			// 获取该客户端的进度（目前已经支持传递 targetID 和 dbName）
			progress, err := s.progressService.GetTaskProgress(taskID, p.TargetID, p.Database)
			if err != nil {
				return
			}
