...
func (c *IncrementalConsumer) Start() error {
	...
	// 初始化任务统计
	c.statsService.InitTaskStats(c.taskID, c.targetID)
	...
	if c.config != nil {
		for _, dbSel := range c.config.SelectedDatabases {
			...
			for _, tbl := range dbSel.Tables {
				...
				// 为每个表创建初始统计（使用目标表名）
				c.statsService.InitTableStats(c.taskID, c.targetID, targetDB, targetTable)
			}
		}
	}
...
func (c *IncrementalConsumer) consumeLoop() {
	...
	c.logService.Info(c.taskID, "增量消费循环开始")
	...
	for {
		...
		default:
			...
			c.logService.Info(c.taskID, fmt.Sprintf("从队列获取到事件: %s %s.%s",
			...
			if err := c.processEvent(event); err != nil {
				...
				c.statsService.UpdateTaskStats(c.taskID, c.targetID, event.Type, event.BinlogFile, event.BinlogPos, eventTime, true)
				...
				c.eventsProcessed++
				c.logService.Info(c.taskID, fmt.Sprintf("事件处理成功: %s %s.%s (总计: %d)",
					event.Type, event.Database, event.Table, c.eventsProcessed))
				...
				if !skip {
					// 更新统计（使用目标表名）
					eventTime := time.Unix(int64(event.Timestamp), 0)
					c.statsService.UpdateTaskStats(c.taskID, c.targetID, event.Type, event.BinlogFile, event.BinlogPos, eventTime, false)
					c.statsService.UpdateTableStats(c.taskID, c.targetID, targetDB, targetTable, event.Type, eventTime)
			...
			if err := c.queue.Ack(event.ID); err != nil {
				...
				c.logService.Info(c.taskID, fmt.Sprintf("事件ACK成功: %s", event.ID))
			}
		}
	}
}
...
func (c *IncrementalConsumer) GetStatistics() map[string]interface{} {
	return map[string]interface{}{
		...
		"save_interval":    c.saveInterval.String(),
		"last_save_time":   c.lastSaveTime.Format("2006-01-02 15:04:05"),
	}
}
