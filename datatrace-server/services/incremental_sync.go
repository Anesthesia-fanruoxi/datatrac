...
func (s *IncrementalSync) GetStatus() map[string]interface{} {
	...
	if s.queue != nil {
		if size, err := s.queue.Size(); err == nil {
			...
			status["queue_size_error"] = err.Error()
		}
	}
	...
	// 获取任务状态
	if s.task != nil {
		status["task_status"] = map[string]interface{}{
			"is_running":           s.task.IsRunning,
			"current_step":         s.task.CurrentStep,
			"full_sync_completed":  s.task.FullSyncCompleted,
			"queue_type":           s.task.QueueType,
			"incremental_enabled":  s.task.IncrementalEnabled,
			"snapshot_binlog_file": s.task.SnapshotBinlogFile,
			"snapshot_binlog_pos":  s.task.SnapshotBinlogPos,
			"current_binlog_file":  s.task.CurrentBinlogFile,
			"current_binlog_pos":   s.task.CurrentBinlogPos,
		}
	}
	...
	status["components"] = map[string]interface{}{
		...
		"consumer_active":     s.consumer != nil,
		"queue_active":        s.queue != nil,
		"source_db_connected": s.sourceDB != nil,
		"target_db_connected": s.targetDB != nil,
	}
