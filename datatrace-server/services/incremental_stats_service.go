...
func (s *IncrementalStatsService) getTaskStatsKey(taskID, targetID string) string {
	return fmt.Sprintf("incremental:task:%s:target:%s:stats", taskID, targetID)
}

// getTableStatsKey 获取表统计的 Redis key
func (s *IncrementalStatsService) getTableStatsKey(taskID, targetID, database, table string) string {
	return fmt.Sprintf("incremental:task:%s:target:%s:table:%s.%s", taskID, targetID, database, table)
}

// getTableListKey 获取表列表的 Redis key
func (s *IncrementalStatsService) getTableListKey(taskID, targetID string) string {
	return fmt.Sprintf("incremental:task:%s:target:%s:tables", taskID, targetID)
}

// InitTaskStats 初始化任务统计
func (s *IncrementalStatsService) InitTaskStats(taskID, targetID string) error {
	...
	key := s.getTaskStatsKey(taskID, targetID)
...
func (s *IncrementalStatsService) InitTableStats(taskID, targetID, dbName, tableName string) error {
	if database.RedisClient == nil {
		return fmt.Errorf("Redis 客户端未初始化")
	}

	key := s.getTableStatsKey(taskID, targetID, dbName, tableName)
	...
	// 添加到表列表
	listKey := s.getTableListKey(taskID, targetID)
...
func (s *IncrementalStatsService) UpdateTableStats(taskID, targetID, dbName, tableName, eventType string, eventTime time.Time) error {
	if database.RedisClient == nil {
...
func (s *IncrementalStatsService) UpdateTaskStats(taskID, targetID, eventType string, binlogFile string, binlogPos uint32, eventTime time.Time, failed bool) error {
	if database.RedisClient == nil {
		return nil
	}
...
func (s *IncrementalStatsService) GetTaskStats(taskID, targetID string) (*IncrementalTaskStats, error) {
	// 默认统计对象
...
func (s *IncrementalStatsService) GetTableStatsList(taskID, targetID string) ([]*IncrementalTableStats, error) {
	if database.RedisClient == nil {
	...
	for _, tableKey := range tables {
		// tableKey 格式为 "db.table"
		key := fmt.Sprintf("incremental:task:%s:target:%s:table:%s", taskID, targetID, tableKey)
...
func (s *IncrementalStatsService) ClearTaskStats(taskID, targetID string) error {
	if database.RedisClient == nil {
		return nil
	}

	// 删除任务统计
	taskKey := s.getTaskStatsKey(taskID, targetID)
...
func (s *IncrementalStatsService) GetDatabaseStatsList(taskID, targetID string) ([]*IncrementalDatabaseStats, error) {
	// 先获取所有表的统计
	tableStats, err := s.GetTableStatsList(taskID, targetID)
	if err != nil {
...
func (s *IncrementalStatsService) GetTableStatsListByDatabase(taskID, targetID, database string) ([]*IncrementalTableStats, error) {
	// 获取所有表统计
	allTableStats, err := s.GetTableStatsList(taskID, targetID)
	if err != nil {
