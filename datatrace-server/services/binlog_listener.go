...
func (l *BinlogListener) Start() error {
	...
	l.logService.Info(l.taskID, fmt.Sprintf("创建Binlog同步器配置 - Host: %s:%d, ServerID: %d, User: %s",
		l.host, l.port, l.serverID, l.username))
	...
	// 输出监听的表信息
	l.logService.Info(l.taskID, "监听的数据库和表:")
	for db, tables := range l.tables {
		if len(tables) == 0 {
			l.logService.Info(l.taskID, fmt.Sprintf("  数据库: %s (所有表)", db))
		} else {
			for table := range tables {
				l.logService.Info(l.taskID, fmt.Sprintf("  数据库.表: %s.%s", db, table))
			}
		}
	}
	...
	l.logService.Info(l.taskID, "Binlog同步器启动成功，开始事件处理循环")
...
func (l *BinlogListener) handleInsert(ev *replication.BinlogEvent) error {
	...
	if !l.shouldProcess(database, table) {
		// 记录跳过的事件（调试用）
		l.logService.Info(l.taskID, fmt.Sprintf("跳过INSERT事件: %s.%s (不在监听范围)", database, table))
		return nil
	}

	l.logService.Info(l.taskID, fmt.Sprintf("处理INSERT事件: %s.%s, 行数: %d", database, table, len(rowsEvent.Rows)))
	...
	for i, row := range rowsEvent.Rows {
		...
		l.logService.Info(l.taskID, fmt.Sprintf("INSERT事件已推送到队列: %s.%s, 行 %d/%d",
			database, table, i+1, len(rowsEvent.Rows)))
	}
...
func (l *BinlogListener) handleUpdate(ev *replication.BinlogEvent) error {
	...
	if !l.shouldProcess(database, table) {
		l.logService.Info(l.taskID, fmt.Sprintf("跳过UPDATE事件: %s.%s (不在监听范围)", database, table))
		return nil
	}

	l.logService.Info(l.taskID, fmt.Sprintf("处理UPDATE事件: %s.%s, 行数: %d", database, table, len(rowsEvent.Rows)/2))
	...
	for i := 0; i < len(rowsEvent.Rows); i += 2 {
		...
		l.logService.Info(l.taskID, fmt.Sprintf("UPDATE事件已推送到队列: %s.%s, 行对 %d/%d",
			database, table, (i/2)+1, len(rowsEvent.Rows)/2))
	}
...
func (l *BinlogListener) handleDelete(ev *replication.BinlogEvent) error {
	...
	if !l.shouldProcess(database, table) {
		l.logService.Info(l.taskID, fmt.Sprintf("跳过DELETE事件: %s.%s (不在监听范围)", database, table))
		return nil
	}

	l.logService.Info(l.taskID, fmt.Sprintf("处理DELETE事件: %s.%s, 行数: %d", database, table, len(rowsEvent.Rows)))
	...
	for i, row := range rowsEvent.Rows {
		...
		l.logService.Info(l.taskID, fmt.Sprintf("DELETE事件已推送到队列: %s.%s, 行 %d/%d",
			database, table, i+1, len(rowsEvent.Rows)))
	}
