...
// targetID 参数可选，如果提供则返回该目标的统计信息，否则默认返回主目标
// dbName 参数可选，如果提供则同时返回该数据库的表明细
func (s *TaskProgressService) GetTaskProgress(taskID string, targetID string, dbName string) (*TaskProgress, error) {
	// 查询任务
	...
	// 始终尝试填充通用全量进度（如果内存中存在数据）
	s.fillFullSyncProgress(taskID, &task, progress)

	// 根据同步模式返回不同的数据
	if task.SyncMode == "incremental" {
		// 增量同步模式：获取当前选中数据源的数据库级别统计，并合并到当前目标源中
		databaseStats := s.getIncrementalDatabaseStats(taskID, targetID, &task)
		progress.TargetStats = s.mergeDatabaseStatsToTargets(progress.TargetStats, &task, targetID, databaseStats)

		// 如果指定了 dbName 参数，则返回该数据库的表明细
		if dbName != "" {
			tableStats, _ := s.GetTaskProgressByDatabase(taskID, targetID, dbName)
...
// mergeDatabaseStatsToTargets 将当前选中目标的数据合并到 target_stats 中
func (s *TaskProgressService) mergeDatabaseStatsToTargets(base []*TargetProgress, task *models.SyncTask, selectedTargetID string, databaseStats []*IncrementalDatabaseStats) []*TargetProgress {
	// 解析任务配置获取所有目标源
	var config struct {
	...
	// 如果配置中没有 target_ids，使用主目标
	targetIDs := config.TargetIDs
	if len(targetIDs) == 0 && task.TargetID != "" {
		targetIDs = []string{task.TargetID}
	}

	baseMap := make(map[string]*TargetProgress, len(base))
	for _, item := range base {
		if item != nil {
			baseMap[item.TargetID] = item
		}
	}

	var targetStats []*TargetProgress
	for _, tID := range targetIDs {
		// 获取目标源名称
		var ds models.DataSource
		targetName := tID
		if err := database.DB.First(&ds, "id = ?", tID).Error; err == nil {
			targetName = ds.Name
		}
		...
		// 只有当前选中的数据源才包含 database_stats
		if tID == selectedTargetID {
			target.DatabaseStats = databaseStats
		} else {
			target.DatabaseStats = nil
		}

		targetStats = append(targetStats, target)
	}
...
func (s *TaskProgressService) getIncrementalDatabaseStats(taskID, targetID string, task *models.SyncTask) []*IncrementalDatabaseStats {
	statsService := NewIncrementalStatsService()

	// 从Redis获取数据库级别统计
	dbStats, _ := statsService.GetDatabaseStatsList(taskID, targetID)
...
func (s *TaskProgressService) getIncrementalTableStats(taskID, targetID string, task *models.SyncTask) []*IncrementalTableStats {
	statsService := NewIncrementalStatsService()

	// 先从Redis获取表统计
	tableStats, _ := statsService.GetTableStatsList(taskID, targetID)
...
func (s *TaskProgressService) GetTaskProgressByDatabase(taskID, targetID, dbName string) ([]*IncrementalTableStats, error) {
	// 查询任务
	var task models.SyncTask
	if err := database.DB.First(&task, "id = ?", taskID).Error; err != nil {
		return nil, fmt.Errorf("任务不存在")
	...
	// 从Redis获取指定数据库的表统计
	tableStats, _ := statsService.GetTableStatsListByDatabase(taskID, targetID, dbName)
