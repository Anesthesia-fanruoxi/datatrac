...
func (s *IncrementalSync) createQueue() error {
	...
	s.logService.Info(s.taskID, fmt.Sprintf("队列类型: %s", queueType))

	var err error
	if queueType == "redis" {
		s.logService.Info(s.taskID, "创建Redis队列...")
		s.queue, err = NewRedisQueue(database.RedisClient, s.taskID, "worker-1")
		if err != nil {
			return fmt.Errorf("创建Redis队列失败: %w", err)
		}
		s.logService.Info(s.taskID, "Redis队列创建成功")
	} else {
		s.logService.Info(s.taskID, "创建内存队列...")
		s.queue = NewMemoryQueue(10000) // 缓冲区大小10000
		s.logService.Info(s.taskID, "内存队列创建成功，缓冲区大小: 10000")
	}
...
func (s *IncrementalSync) executeFullSync() error {
	...
	statsService.ClearTaskStats(s.taskID, s.task.TargetID)
	...
	for _, db := range config.SelectedDatabases {
		...
		for _, tableConfig := range db.Tables {
			...
			statsService.InitTableStats(s.taskID, s.task.TargetID, targetDB, targetTable)
		}
	}
...
func (s *IncrementalSync) startIncrementalConsumer() error {
	...
	s.logService.Info(s.taskID, fmt.Sprintf("连接目标数据库: %s:%d",
		s.task.TargetConn.Host, s.task.TargetConn.Port))
	...
	s.logService.Info(s.taskID, "目标数据库连接成功")

	// 创建消费者配置
	consumerConfig := &IncrementalConsumerConfig{
		TaskID:        s.taskID,
		TargetID:      s.task.TargetID,
		Queue:         s.queue,
		TargetDB:      targetDB,
	...
	s.logService.Info(s.taskID, fmt.Sprintf("消费者配置 - 错误策略: %s, 保存间隔: %v",
		consumerConfig.ErrorStrategy, consumerConfig.SaveInterval))
