...
// GetConfigView 获取任务配置视图（包含实时表结构）
func (api *TaskAPI) GetConfigView(c *gin.Context) {
	id := c.Param("id")
	configView, err := api.service.GetConfigWithRealTimeStructure(id)
	if err != nil {
		common.NotFound(c, "任务不存在或获取配置失败: "+err.Error())
		return
	}

	common.Success(c, configView)
}
